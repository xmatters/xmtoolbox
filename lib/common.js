    ...
    sourceObjects.map(async sourceObject => {
      ...
      if (match) {
        ...
        if (_.isMatch(destObjectProps, sourceObjectProps)) {
          ...
          const diff = _.reduce(
            destObjectProps,
            (result, value, key) => (_.isEqual(value, sourceObjectProps[key]) ? result : result.concat(key)),
            []
          );
        ...
        //Initial will set regardless of prior value.
        let record = _.defaults(sourceObject, sourceObject.default);
        record = _.defaults(sourceObject.initial, sourceObject);
...
function convertDefaultInitial(objects, convert) {
  objects.map(object => {
    object.default = convert(object.default);
    object.initial = convert(object.initial);
