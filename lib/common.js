...
async function syncObject(
  ...
  const deleteShiftsOnCreate = options.deleteShiftsOnCreate || false;
      ...
      if (match) {
        ...
        if (_.isMatch(destObjectProps, sourceObjectProps)) {
          ...
          if (typeof updateFunction === 'function') {
            ...
            return created.push(createFunction(destination, update, parentId));
          }
        }
      } else {
        ...
        return created.push(await createFunction(destination, sourceObject, parentId));
      }
    })
  );
  ...
  return { synced, remove: pendingDelete, created };
}
