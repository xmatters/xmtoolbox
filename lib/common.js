 ...
 */
function requestList(total, auth, uri, throwError, proxy, timeout) {
  const json = null;
  const pageSize = 100;
  const method = 'GET';
...
async function get(
  ...
  const uri = `${baseUrl}${api}${encodeURIComponent(id)}?${queryString(query)}`;
...
async function getMany(
  ...
  const options = { method: 'GET', uri, auth, throwError, proxy, timeout };
...
async function create(
  ...
  //HM-XM01182023: changing if statement for subscription share permission object because they do not have an id
  if (id || body.data.length > 0) {
    if(label === 'Subscription Share Permissions'){
      log.debug('XMAPI:', subdomain, label, 'Created', name, '(no id for Subscription Share object)');
    }else{
...
async function _delete(
  ...
  const options = { method: 'DELETE', uri, auth, throwError, proxy, timeout: 45000 };
  ...
  const synced = await Promise.all(
    sourceObjects.map(async sourceObject => {
      ...
      if (match) {
        ...
        let skip = false;
        if (name === 'Person' && fields.includes('properties')) {
          //is person and includes properties, check the properties for isMatch
          if (isMatch(destObjectProps.properties, sourceObjectProps.properties)) {
            //is match for properties, check other keys for isEqual match
            const _fields = fields.filter(field => field !== 'properties');
            const _destObjectProps = pick(destObjectProps, _fields);
            const _sourceObjectProps = pick(sourceObjectProps, _fields);
            if (isEqual(_destObjectProps, _sourceObjectProps)) {
              skip = true;
            }
          }
        } else if (isEqual(destObjectProps, sourceObjectProps)) {
          skip = true;
        }
...
function convertDefaultInitial(objects, convert) {
  return objects.map(object => {
    ...
    if (object.initial && typeof object.initial === 'object') object.initial = convert(object.initial);
