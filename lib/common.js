 ...
 * @name delete
 */
async function _delete({ baseUrl, limiter, auth, subdomain, log, readOnly }, api, id, label = '') {
 ...
 */
async function update({ baseUrl, limiter, auth, subdomain, log, readOnly }, api, json, id, label = '') {
  const name = json.targetName || json.name || '';
  const uri = `${baseUrl}${api}`;
  json.id = id;
  const options = { method: 'POST', uri, auth, json };
...
async function request({ baseUrl, limiter, auth, subdomain, log }, api, query, options, label = '') {
  let uri = `${baseUrl}${api.replace(baseUrl, '')}?${queryString(query)}`;
  options.uri = uri;
  options.auth = auth;
 ...
 */
  ...
  if (!response)
    throw new Error(
      `The response from xMatters was invalid after multiple attempts. Pleae review the output above for more details. Related request: ${options.method} ${options.uri}`
    );
  ...
  const synced = await Promise.all(
    sourceObjects.map(async sourceObject => {
      ...
      if (match) {
        ...
        created.push(await createFunction(destination, sourceObject, parentId));
