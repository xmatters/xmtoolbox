...
async function get(env, deviceId, query) {
  return common.get(env, '/api/xm/1/devices/', deviceId, query, 'Device');
}
...
async function _delete(env, deviceId) {
  await common.delete(env, '/api/xm/1/devices/', deviceId, 'Device');
}
  ...
  return devices.map(device => {
    {
      ...
      //delete record.owner;
      device.owner = destinationPeople.find(
 ...
 * @param {module:environments.xMattersEnvironment} destination The xmtoolbox representation of the target or destination xMatters instance.Devices
 * @param {Object} options
