...
async function getManyUserDeliveries(env, query, eventId) {
  return util.getMany(env, `/api/xm/1/events/${eventId}/user-deliveries`, query, 'Event User Delivervies');
}
...
async function create(env, event, functionId) {
  return util.create(env, `/api/integration/1/functions/${functionId}/triggers`, event, 'Event', true);
}
