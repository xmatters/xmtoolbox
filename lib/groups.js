...
async function getSupervisors(env, query, groupId) {
  return common.getMany(
    env,
    `/api/xm/1/groups/${encodeURIComponent(groupId)}/supervisors`,
    query,
    'Group Supervisors'
  );
}
...
async function exportToImport(destination, groups, destinationData, options) {
  ...
  return common.convertDefaultInitial(groups, convert);
  ...
  function convert(group) {
    {
      ...
      if (group.site && group.site.name) {
        let site = group.site;
        site = destinationSites && destinationSites.find(({ name }) => group.site.name === name);
        if (site) group.site = site.id;
      }

      if (group.observers) {
        //goal format: observers: [{name:"Company Supervisor"}]
        //from xMatters
        if (group.observers.data) {
          group.observers = group.observers.data;
          group.observers = group.observers.map(({ name }) => ({ name }));
        } else if (Array.isArray(group.observers)) {
          //handle array of strings
          group.observers = group.observers.map(observer => {
      ...
      if (group.supervisors) {
        ...
        group.supervisors = group.supervisors.map(supervisor => {
          const destinationSupervisor =
            destinationPeople &&
            destinationPeople.find(({ targetName }) => targetName === (supervisor.targetName || supervisor));
