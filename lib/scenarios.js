...
async function getMany(env, query, formId, planId) {
  return common.getMany(
    env,
    `/api/xm/1/plans/${encodeURIComponent(planId)}/forms/${formId}/scenarios`,
    query,
    'Scenarios'
  );
}
...
async function create(env, scenario, formId) {
  return common.create(
    env,
    `/api/xm/1/forms/${encodeURIComponent(formId)}/scenarios`,
    scenario,
    'Scenario',
    true
  );
...
function exportToImport(destination, scenarios, destinationData) {
  ...
  return common.convertDefaultInitial(scenarios, convert);
