 ...
 * @property {boolean} [services=false] Whether or not to synchronize services
 * @property {function} [servicesFilter=undefined] Array.filter(callback) function to apply to the services arrays from the source and destination after data is extracted.<br><br> Example: o => o.foo === 'bar'' //filters out all but the scenario objects where the foo property is equal to 'bar'. <br><br> Refer to the javascript Array.filter() documentation for more information.
 * @property {Object} [servicesQuery={ embed: 'properties.translations' }] Object representation of the query parameters supported by the Get many API request for services
...
async function ExtractData(env, extractOptions, syncOptions) {
  ...
  if (extractOptions.services) {
    if (service) {
      const identifier = service.id || service.targetName|| person;
      const _service = await xm.services.get(env, identifier, servicesQuery, false);
      data.service = _service ? [_service] : [];
    } else {
      data.services = await xm.services.getMany(env, servicesQuery);
    }
  }
...
async function ConvertAndSyncObjects(sourceData, destinationData, env, syncOptions) {
  ...
  const _servicesOptions = xm.common.defaultsDeep(servicesOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
  ...
  const objectNames = [
    ...
    [services, 'services', servicesFilter, servicesTransform, _servicesOptions]
  ];
