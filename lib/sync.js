 ...
 * @type {Object}
 *
 * @property {boolean} [sameAsSource=false] Whether or not the data going into xMatters is from the same instance. This is useful to avoid unnecessary data lookups for UUIDs when modifying data in for a single xMatters instance.
 ...
 * @property {Object} [peopleQuery={ embed: 'roles,supervisors' }, //possible to include ',devices' but simpler to extract devices separately] Object representation of the query parameters supported by the Get many API request for people
...
function DefaultSyncOptions() {
  return {
    ...
    people: false,
    peopleQuery: { embed: 'roles,supervisors' }, //possible to include ',devices' but simpler to extract devices separately.
...
async function xMattersToxMatters(sourceEnv, destinationEnv, syncOptions = {}) {
  const { mirror, dataExtracted } = _.defaultsDeep(syncOptions, DefaultSyncOptions());
...
async function DataToxMatters(sourceData, destinationEnv, syncOptions = {}) {
  ...
  const { sameAsSource, dataExtracted } = syncOptions;
  const isDestination = sameAsSource ? false : true;
  ...
  const destinationData = await ExtractData(
    destinationEnv,
    GetExtractOptions(syncOptions, isDestination),
    syncOptions
  );
  ...
  if (extractOptions.forms && data.plans) {
    const plansForms = await Promise.all(
      data.plans.map(async ({ id }) => {
        return xm.forms.getManyInPlan(env, formsQuery, id);
      })
    );
...
async function ConvertAndSyncObjects(sourceData, destinationData, env, syncOptions) {
  ...
  const _sitesOptions = _.defaultsDeep(sitesOptions, { mirror, delayRemoval, continueOnError });
  const _peopleOptions = _.defaultsDeep(peopleOptions, { mirror, delayRemoval, continueOnError });
  const _devicesOptions = _.defaultsDeep(devicesOptions, { mirror, delayRemoval: false, continueOnError });
  const _groupMembersOptions = _.defaultsDeep(groupMembersOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
  ...
  const _shiftsOptions = _.defaultsDeep(shiftsOptions, { mirror, delayRemoval: false, continueOnError });
  ...
  const _temporaryAbsencesOptions = _.defaultsDeep(temporaryAbsencesOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
  const _integrationsOptions = _.defaultsDeep(integrationsOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
  const _plansOptions = _.defaultsDeep(plansOptions, { mirror, delayRemoval: false, continueOnError });
  const _planConstantsOptions = _.defaultsDeep(planConstantsOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
  const _planEndpointsOptions = _.defaultsDeep(planEndpointsOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
  const _planPropertiesOptions = _.defaultsDeep(planPropertiesOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
  const _sharedLibrariesOptions = _.defaultsDeep(sharedLibrariesOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
  const _scenariosOptions = _.defaultsDeep(scenariosOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
  const _subscriptionsOptions = _.defaultsDeep(subscriptionsOptions, {
    mirror,
    delayRemoval: false,
    continueOnError,
  });
      ...
      if (syncResults[objectName] && syncResults[objectName].remove) {
        ...
        await Promise.all(
          syncResults[objectName].remove.map(async object => {
            syncResults[objectName].deleted.push(await xm[objectName].delete(env, object.id));
          })
        );
      }
    })
