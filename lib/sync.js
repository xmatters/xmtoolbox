...
function GetExtractOptions(syncOptions, isDestination) {
  ...
  if (syncForms || syncScenarios || syncIntegrations) get.forms = true;
...
async function ConvertAndSyncObjects(sourceData, destinationData, env, syncOptions) {
  ...
  if (!syncPersonSupervisors) peopleFields = peopleFields.filter(e => e !== 'supervisors');
  ...
  await Promise.all(
    objectNames.map(async ([shouldSync, objectName, filter, transform, options]) => {
      if (shouldSync) {
        return await ConvertAndSyncObject(
          sourceData,
          destinationData,
          env,
          objectName,
