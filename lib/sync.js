...
async function ExtractData(env, extractOptions, syncOptions) {
  ...
  if (extractOptions.devices) {
    if (person) {
      ...
      data.devices = await xm.people.getDevices(env, devicesQuery, identifier);
    } else if (device) {
      const identifier = device.targetName || device.id || device;
      data.devices = [await xm.devices.get(env, devicesQuery, identifier, false)];
    } else {
  ...
  if (extractOptions.plans) {
    if (plan) {
      ...
      data.plans = [await xm.plans.get(env, identifier, plansQuery)];
    } else {
  ...
  if (extractOptions.groups) {
    if (group) {
      ...
      data.groups = [await xm.groups.get(env, identifier, groupsQuery, false)];
    } else {
  ...
  if (extractOptions.people) {
    if (person) {
      const identifier = person.targetName || person.id || person;
      data.people = [await xm.people.get(env, identifier, peopleQuery, false)];
    } else {
  ...
  if (extractOptions.sites) {
    if (extractOptions.people) {
      if (site) {
        ...
        data.sites = [await xm.sites.get(env, identifier, sitesQuery, false)];
      } else {
...
async function MirrorOrderedDelete(env, syncResults) {
  ...
  objectNames.map(async objectName => {
    if (syncResults[objectName] && syncResults[objectName].pendingDelete) {
      ...
      await Promise.all(
        syncResults[objectName].pendingDelete.map(async object => {
